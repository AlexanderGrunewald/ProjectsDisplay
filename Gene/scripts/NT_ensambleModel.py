...
import numpy as np
import time
from torchensemble import NeuralForestRegressor
import os
...
if __name__ == "__main__":
    ...
    with open(model_log, "a") as f:
        ...
        f.write(
            f"Neural Forest Regressor Ensemble Model: n_estimators={n_estimators}, lr={lr}, weight_decay={weight_decay}, epochs={epochs}\n"
        )

    # Create ensemble model
    model = NeuralForestRegressor(
        estimator=MLP, n_estimators=n_estimators, cuda=True, n_jobs=4
    )
    ...
    with open(model_log, "a") as f:
        ...
        f.write(f"Evaluating time: {evaluating_time/60}\n\n")
        # end time
        f.write(f"End Time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}\n\n")

    torch.save(model, "/mnt/home/grunew14/Documents/ss-23-classes/cmse381/gene-project/trained_models/NT_ensambleModel.pt")
    print("Model saved.")
